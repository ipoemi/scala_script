...
object QueryHelper {
  ...
  private val dbInfoMap = Map(
    ...
    "logis_yj" -> Map("url" -> "jdbc:oracle:thin:@210.107.83.2:1521:ORA10g", "username" -> "sjlgs_yj", "password" -> "sjlgs_yj"))
  ...
  def getDeleteTrigger(dbName: String, username: String, tableName: String): String = {
    ...
    connTry.map { connection =>
      ...
      val triggerName =
        if (dbName.toLowerCase == "sjlgs" || dbName.toLowerCase == "logis_sj" || dbName.toLowerCase() == "logis_yj") {
          s"LOG_${tableName}_D"
        } else {
          s"${tableName}_D_B_T"
        }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val tableName = "행사반품검수상세"
    println(getHistoryTable("logis_sj", "logis_sj", tableName))
    println(getDeleteTrigger("logis_sj", "logis_sj", tableName))
    println(getUpdateTrigger("logis_sj", "logis_sj", tableName))
